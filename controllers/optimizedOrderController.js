...
exports.getPaginatedOrders = async (req, res) => {
  try {
    ...
    const TIMEZONE_OFFSET_HOURS = 3; // Bahrain = UTC+3

const todayStart = new Date();
todayStart.setUTCHours(0 - TIMEZONE_OFFSET_HOURS, 0, 0, 0);

const todayEnd = new Date();
todayEnd.setUTCHours(23 - TIMEZONE_OFFSET_HOURS, 59, 59, 999);
...
if (dateFilter === 'Today') {
  ...
  start.setUTCHours(0 - TIMEZONE_OFFSET_HOURS, 0, 0, 0);

  const end = new Date(endDate);
  end.setUTCHours(23 - TIMEZONE_OFFSET_HOURS, 59, 59, 999);
...
if (dateFilter === 'Today') {
  baseFilter.createdAt = { $gte: todayStart, $lte: todayEnd };
  ...
  baseFilter.createdAt = { $gte: yesterdayStart, $lte: yesterdayEnd };
  console.log('📅 [YESTERDAY FILTER]');
  console.log('  Start (UTC):', yesterdayStart.toISOString());
  console.log('  End   (UTC):', yesterdayEnd.toISOString());
...
exports.getPaymentTotals = async (req, res) => {
  ...
  const start = new Date(startDate);
  start.setUTCHours(0 - TIMEZONE_OFFSET_HOURS, 0, 0, 0); // 21:00 prev day UTC = midnight Bahrain

  const end = new Date(endDate);
  end.setUTCHours(23 - TIMEZONE_OFFSET_HOURS, 59, 59, 999); // 20:59 UTC = 23:59 Bahrain
